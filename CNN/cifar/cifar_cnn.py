...
def next_test_batch(step=0, batch_size=128):
    ...
    batch_y = []
    for i in range(step * batch_size, (step + 1) * batch_size):
        batch_x.append(x_test[i % len(x_test)])
        batch_y.append(y_test[i % len(y_test)])
    batch_x = np.array(batch_x)
    batch_y = np.array(batch_y)
    return batch_x, batch_y
...
def cnn(input_tensor):
    ...
    # 全连接层
    W_fc1 = weight_variable([8 * 8 * 32, 100])
    b_fc1 = bias_variable([100])
    h_pool2_flat = tf.reshape(h_pool2, [-1, 8 * 8 * 32])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
    h_fc1 =tf.nn.dropout(h_fc1,FLAGS.dropout)
    # 输出层
...
def optimization(name, cross_entropy):
    ...
    learning_rate_decayed = FLAGS.learning_rate / tf.pow(global_step,0.2)
    if name == "ADAM":
        train_step = AdamOptimizer(FLAGS.learning_rate).minimize(cross_entropy, global_step=global_step)
    elif name == "AMSGRAD":
...
def iterations(input_tensor, labels, loss, accuracy, optimizate):
    ...
    for i in range(0,FLAGS.max_steps):
        ...
        if (i % 201 == 1):
            ...
            # batch_xs, batch_ys = next_test_batch(i-1)
            test_loss_, test_acc_ = sess.run([loss, accuracy], feed_dict={input_tensor: x_test, labels: y_test})
